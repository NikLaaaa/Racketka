...
(() => {
  ...
  const state = {
    ...
    balance: 0,
    stars: 0,
    roundState: "idle", // betting/running/finished
    displayedMult: 1,
    myBet: 0,
    myCashed: false
  };
  ...
  function readTelegramUser() {
    ...
    // fallback из query (когда открыли не из TG)
    const q = new URLSearchParams(location.search);
    if (!u && (q.get("name") || q.get("photo"))) {
      u = {
        first_name: q.get("name") || "User",
        last_name: "",
        username: q.get("username") || null,
        photo_url: q.get("photo") || null
      };
    }
    return u;
  }
  ...
  function fillProfileFromTelegram() {
    ...
    if (profileAva) photo ? (profileAva.src = photo) : profileAva.removeAttribute("src");

    return { nick: displayName, avatar: photo || null };
  }

  function sendProfile(ws) {
    const p = fillProfileFromTelegram();
    try {
      if (ws.readyState === 1) {
        ws.send(JSON.stringify({ type: "profile", profile: p }));
      }
    } catch { }
  }

  document.addEventListener("DOMContentLoaded", fillProfileFromTelegram);
  ...
  ws.addEventListener("open", () => {
    sendProfile(ws);              // сразу
    setTimeout(() => sendProfile(ws), 400);
    setTimeout(() => sendProfile(ws), 1500);
  });
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) sendProfile(ws);
  });
  ...
  ws.onmessage = (ev) => {
    ...
    switch (d.type) {
      ...
      case "bet_confirm":
        ...
        betBtn.disabled = false; // можно снова открыть модалку, ограничений нет
      ...
      case "profile_update":
        // только обновляем отображение у уже существующего игрока, без добавления фантомов
        if (players.has(d.userId)) {
          const p = players.get(d.userId);
          p.nick = d.nick || p.nick;
          p.avatar = (d.avatar ?? p.avatar) || null;
          renderPlayers();
        }
        break;
  ...
  function setBalance(v) {
    ...
    if (balanceNumEl) balanceNumEl.textContent = state.balance.toFixed(2);
    if (profileBalance) profileBalance.textContent = state.balance.toFixed(2);
  }
  function setStars(n) {
    state.stars = Number(n) || 0;
    const el = $("#starsBalance");
  ...
  function ensureBetInputReady(initialValue = "") {
    ...
    const tries = [0, 60, 120, 240];
    tries.forEach(t => setTimeout(() => { modalBetInput.focus(); modalBetInput.select(); }, t));
    modalBetInput.addEventListener("touchstart", () => { modalBetInput.focus(); modalBetInput.select(); }, { once: true });
  }
  ...
  function openModal(el) {
    ...
    (modalBox || el.firstElementChild)?.addEventListener("click", e => e.stopPropagation());
  ...
  function openBetModal() {
    // разрешаем ввод ВСЕГДА; просто предупредим, если ставки закрыты
    if (state.roundState !== "betting") {
      setTimeout(() => alert("Ставки пока закрыты. Введите сумму — подтвердите, когда начнётся новый раунд."), 0);
    }
    ensureBetInputReady(modalBetInput.value || "");
    openModal(betModal);
  }
  ...
  modalConfirm?.addEventListener("click", () => {
    ...
    if (state.roundState !== "betting") {
      alert("Ставки ещё закрыты. Дождитесь начала раунда и нажмите «Поставить».");
      ensureBetInputReady(raw);
      return;
    }
    ...
    placingBet = true;
    try {
      const prof = fillProfileFromTelegram();
  ...
  function setTab(name) {
    ...
    if (isWheel) wheelRender(); // отрисовать при показе
  }
  ...
  function onRoundStart(d) {
    ...
    betBtn.disabled = false;         // НЕ блокируем кнопку — инпут доступен всегда
    cashoutBtn.disabled = true;
    cashoutBtn.classList.remove("active", "pulse", "armed");
    payoutTopEl.textContent = "";
  ...
  // ================== WHEEL (5 NFT, 2 No Loot, 1 Re-roll) + Stars ==================
  ...
  function syncStars(n) { WSTATE.stars = Number(n || state.stars || 0); if (starsEl) starsEl.textContent = WSTATE.stars; }
  syncStars(state.stars);
  ...
  function wheelRender() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      // разделитель
      wctx.strokeStyle = "#0b1119"; wctx.lineWidth = 3;
      wctx.beginPath(); wctx.arc(0, 0, R, a1 - 0.002, a1 + 0.002); wctx.stroke();
      // контент
  ...
  function drawPill(text, accent = false) {
    ...
    wctx.fillStyle = "#dbe7ff"; wctx.textAlign = "center"; wctx.textBaseline = "middle"; wctx.fillText(text, 0, 1);
  }
  ...
  function wheelSpin(pay = true) {
    ...
    if (pay) { WSTATE.stars -= WSTATE.price; syncStars(WSTATE.stars); }
  ...
  // Покупка Stars с корректной оплатой в мини-приложении
  async function buyStarsAndSpin() {
    try {
      ...
      if (!r.ok) { alert(r.error || "Ошибка платежа"); return; }

      const link = r.link;
      const tg = window.Telegram?.WebApp;

      // 1) Нативное окно оплаты TG (мобилки)
      if (tg?.openInvoice) {
        tg.openInvoice(link, async (res) => {
          // res: 'paid' / 'cancelled' / 'failed'
          if (res === 'paid') {
            await confirmStars(r.payload);
            wheelSpin(true);
      ...
      // 2) Fallback (ПК): просто открываем ссылку
  ...
  async function confirmStars(payload) {
    try {
      ...
      if (r.ok) { syncStars(r.stars); return true; }
      return false;
    } catch { return false; }
  }
