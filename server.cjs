...
app.get('/tonconnect-manifest.json', async (_req,res)=>{
  try { res.type('application/json').send(await fs.readFile(path.join(__dirname,'tonconnect-manifest.json'),'utf8')); }
  catch { res.status(404).json({error:'manifest not found'}); }
});
...
const GIFTS_CACHE_TTL=300_000, GIFTS_LIMIT=200;
...
async function gqlRequest(query, variables, {retries=3, timeoutMs=12000}={}){
  ...
  for (let attempt=1; attempt<=retries; attempt++){
    try{
      ...
      const r=await fetch(GETGEMS_GQL,{method:'POST',headers:{'content-type':'application/json','accept':'application/json','origin':'https://getgems.io','referer':'https://getgems.io/','user-agent':'Mozilla/5.0'},body,agent:keepAliveAgent,signal:ctrl.signal});
      clearTimeout(tId);
      if (!r.ok){ if (attempt<retries){ await new Promise(res=>setTimeout(res,400*attempt)); continue; } throw new Error('Getgems HTTP '+r.status); }
      const js=await r.json().catch(()=>({})); if (js.errors){ if (attempt<retries){ await new Promise(res=>setTimeout(res,400*attempt)); continue; } throw new Error('Getgems GraphQL errors'); }
      return js.data;
    }catch(e){ if (attempt>=retries) throw e; await new Promise(res=>setTimeout(res,500*attempt)); }
  }
}
const GQL_FIND_COLLECTIONS=`query FindGiftCollections($q:String!,$limit:Int!){ collections(filter:{search:$q},orderBy:{field:VOLUME,direction:DESC},first:$limit){edges{node{address name}}}}`;
const GQL_COLLECTION_ITEMS=`query Gifts($address:String!,$limit:Int!){ items(filter:{collectionAddress:$address},orderBy:{field:PRICE,direction:ASC},first:$limit){edges{node{address name image{url} bestListing{priceTon} lastSale{priceTon}}}}}`;
  ...
  for (const q of KEYS){
    try{
      ...
      for (const e of (data?.collections?.edges||[])){
        const addr=e?.node?.address; if (!addr || seen.has(addr)) continue;
        seen.add(addr); out.push(addr); if (out.length>=LIMIT) return out;
      }
    }catch{}
  }
  return out;
}
async function fetchCollectionGifts(address, per=200){
  const data=await gqlRequest(GQL_COLLECTION_ITEMS,{address,limit:per});
  const edges=data?.items?.edges||[];
  const items=edges.map(e=>{
    const n=e?.node||{}; const priceTon=Number(n?.bestListing?.priceTon??0)||Number(n?.lastSale?.priceTon??0)||0;
    const img=n?.image?.url||''; return { id:n.address, name:n.name||'Gift', priceTon, img, _col:address };
  });
  return items.filter(x=>x.img && x.priceTon>0).sort((a,b)=>a.priceTon-b.priceTon);
}
async function fetchAllGifts(){
  let col=[]; try{ col=await discoverGiftCollections(); }catch{} if (!col || col.length===0) col=FALLBACK_COLLECTIONS;
  const per=Math.max(1, Math.floor(GIFTS_LIMIT/Math.max(1,col.length)));
  const res=await Promise.allSettled(col.map(a=>fetchCollectionGifts(a,per)));
  const merged=[]; for (const r of res) if (r.status==='fulfilled') merged.push(...r.value);
  const seen=new Set(); const uniq=[]; for (const it of merged){ if (seen.has(it.id)) continue; seen.add(it.id); uniq.push(it); }
  return uniq.filter(x=>x.priceTon>0).sort((a,b)=>a.priceTon-b.priceTon).slice(0,GIFTS_LIMIT);
}

app.get('/gems_health', async (_req,res)=>{ try{ res.json({ok:true, data: await gqlRequest(GQL_PING,{})}); }catch(e){ res.status(500).json({ok:false,error:String(e.message||e)}); }});
app.get('/gifts', async (_req,res)=>{
  try{
    const now=Date.now();
    if (now-giftsCache.ts<GIFTS_CACHE_TTL && giftsCache.items.length) return res.json({ok:true,items:giftsCache.items});
    const items=await fetchAllGifts(); giftsCache={ts:Date.now(),items}; res.json({ok:true,items});
  }catch(e){ console.error('[/gifts] error',e); res.status(500).json({ok:false,error:'getgems fetch failed'}); }
});
...
app.post('/stars/create', async (req,res)=>{
  try{
    ...
    const { userId, amount } = req.body||{}; const stars=Number(amount||0);
    if (!userId || !stars) return res.json({ ok:false, error:'bad params' });
    const payload=`wheel:${userId}:${Date.now()}:${stars}`;
    const body={ title:'Wheel spin', description:`Покупка ${stars} ⭐ для спина`, payload, currency:'XTR', prices:[{label:'⭐', amount:stars}] };
    const r=await fetch(`${TG}/createInvoiceLink`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}).then(r=>r.json());
...
app.post('/stars/credit', async (req,res)=>{
  try{
    ...
    db._creditedStars[payload]=true; await saveDB(db);
    res.json({ ok:true, stars: db.stars[userId] });
  }catch(e){ res.status(500).json({ ok:false, error:String(e.message||e) }); }
});

// ===== GAME (Crash) =====
let online=0, currentMultiplier=1.0, phase='idle'; // betting/running/finished
const clients=new Map(); // ws -> { userId, bet, cashed, nick, avatar }

function broadcast(obj){ const msg=JSON.stringify(obj); wss.clients.forEach(c => (c.readyState===1)&&c.send(msg)); }

function startRound(){
  phase='betting'; currentMultiplier=1.0;
  for (const st of clients.values()) { st.bet=0; st.cashed=false; }
  const BET_MS=5000; const endsAtTs=Date.now()+BET_MS;
  broadcast({ type:'round_start', bettingEndsAt:endsAtTs, betDurationMs:BET_MS });
  setTimeout(runFlight, BET_MS);
}
function runFlight(){
  phase='running'; broadcast({ type:'round_running' });
  const tick=()=>{ if (phase!=='running') return;
...
async function endRound(finalX){
  phase='finished'; const db=await dbPromise;
  db.history.unshift(finalX); db.history=db.history.slice(0,50); await saveDB(db);
...
wss.on('connection', async (ws, req)=>{
  ...
  ws.send(JSON.stringify({
    ...
    players:[], history:db.history||[], wallet:null, online
  }));
  ...
  ws.on('message', async raw=>{
    ...
    if (d.type==='cashout' && phase==='running' && st.bet>0 && !st.cashed){
      ...
      const db=await dbPromise; db.balances[st.userId]=+(Number(db.balances[st.userId]||0)+payout);
      st.bet=0; st.cashed=true; await saveDB(db);
  ...
  ws.on('close', ()=>{ clients.delete(ws); online--; broadcast({ type:'online', online }); });
});
...
async function pollTonCenter(){
  ...
  try{
    const url=`https://toncenter.com/api/v2/getTransactions?address=${encodeURIComponent(DEPOSIT_WALLET)}&limit=20`+(TONAPI_KEY?`&api_key=${encodeURIComponent(TONAPI_KEY)}`:'');
    const res=await fetch(url); const js=await res.json();
    if (js.ok && Array.isArray(js.result)){
      const txs=js.result; const db=await dbPromise;
      for (const tx of txs){
        ...
        if (sender && valueNano>0){
          ...
          if (uid){ const ton=valueNano/1e9; db.balances[uid]=Number(db.balances[uid]||0)+ton; db._creditedTxs[txId]=true; console.log(`+${ton} TON => ${uid}`); }
          else db._creditedTxs[txId]=true;
        }
      }
      await saveDB(db);
    }
